...
def start_test():
 global start_time, random_template, words_index
 start_time = time.time()

 random_template = random.choice(templates)  # randomly choose a word template
 words_index = 0  # keeps track of the current word

 test_text.config(state=tk.NORMAL)
 test_text.delete("1.0", tk.END)
 for word in random_template:
  test_text.insert(tk.END, word + " ")  # display the template words
 test_text.config(state=tk.DISABLED)

 start_button.config(state=tk.DISABLED)
 update_timer(60)  # start the 60-second countdown
 typing_input.bind("<space>", check_word)  # bind space key to check word


# function to check the typed word
def check_word(event):
 global words_index
 typed_text = typing_input.get("1.0", "end-1c").split()  # get typed words
 current_word = typed_text[-1].strip()  # get the last word typed

 test_text.tag_remove("correct", f"1.{words_index}", f"1.{words_index + len(random_template[words_index])}")
 test_text.tag_remove("incorrect", f"1.{words_index}", f"1.{words_index + len(random_template[words_index])}")

 if current_word == random_template[words_index]:  # correct word
  test_text.tag_add("correct", f"1.{words_index}", f"1.{words_index + len(current_word)}")
  test_text.tag_config("correct", background="lime")  # highlight correct word in neon green
 else:
  test_text.tag_add("incorrect", f"1.{words_index}", f"1.{words_index + len(random_template[words_index])}")
  test_text.tag_config("incorrect", background="red")  # highlight incorrect word in red

 words_index += len(random_template[words_index]) + 1  # move to the next word
