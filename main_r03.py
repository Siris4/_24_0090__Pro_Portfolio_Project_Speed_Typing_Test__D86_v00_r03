...
def calculate_speed():
 ...
 correct_count = sum([1 for word in typed_words if word in typed_word_history])
...
def check_word(event):
 ...
 typed_word = typing_input.get("1.0", "end-1c").strip()  # get the word typed by the user
 typing_input.delete("1.0", tk.END)  # clear the input box for the next word
 ...
 words_index = 0 if words_index + len(current_word) >= len(" ".join(current_words)) else words_index + len(
  current_word) + 1
...
def start_test():
 ...
 typing_input.bind("<Return>", check_word)  # bind 'Enter' key to check word
...
typing_input = tk.Text(root, height=2, width=90, bg="white", fg="black", font=("Helvetica", 16))
